...
# java -ea -XX:-UseGCOverheadLimit -Xmx4G -cp "target/classes:$(cat .classpath):lib/com.microsoft.z3.jar:./instrumented:." Problem$1 ${@:2}
...
java -ea -agentpath:$HOME/projects/async-profiler/build/libasyncProfiler.so=start,event=cpu,file=profile.html,title="Problem$1 $(date '+%T')",minwidth=0.2 -XX:-UseGCOverheadLimit -Xmx4G -cp "target/classes:$(cat .classpath):lib/com.microsoft.z3.jar:./instrumented:." Problem$1 ${@:2}
