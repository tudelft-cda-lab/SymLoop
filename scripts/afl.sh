...
prepare () {
    ...
    echo "Compiling $1";
    afl-gcc "$OUT/Problem$1.c" -o "$OUT/Problem$1"
    echo "Fuzzing $1";
    AFL_I_DONT_CARE_ABOUT_MISSING_CRASHES=1 AFL_SKIP_CPUFREQ=1 afl-fuzz -i "$OUT/$1/tests" -o "$OUT/$1/findings" "$OUT/Problem$1"
    # ../AFL/afl-2.52b/afl-plot $OUT/$1/findings/ $OUT/$1/plot


    cd $OLD
    ./scripts/analyze_afl.py "$OUT/$1/findings/default" "$OUT/Problem$1"
