...
prepare () {
    ...
    sed -i -z 's/while(1)\n.*if\(([^\n]*)\)[^}]*}/int length = 50;int program[length];klee_make_symbolic(program, sizeof(program), "program");for (int i = 0; i < length; ++i) {int input = program[i];if(\1){return 0;}calculate_output(input);}/' "$OUT/Problem$1.c"
    ...
    ADDITIONAL_ARGS="--max-time=10min --use-merge"
