...
public class LoopDetection {
    ...
    boolean isIterationLooping() {
        ...
        if (SymbolicExecutionLab.isCreatingPaths && SymbolicExecutionLab.processedInputList.size() == LoopVerifier.saveAtIndex) {
            LoopVerifier.indexBefore = history.getNumberOfSaves();
            LoopVerifier.from = history.getVariables();
        }
        ...
        if (LoopVerifier.loopVerification) {
            lastNSaves = LoopVerifier.loopSize + 1;
        ...
        for (; lastNSaves <= depth; lastNSaves++) {
            ...
            if (lastNSaves > 1 && PathTracker.solve(selfLoopExpr, SolvingForType.IS_SELF_LOOP, false, false)) {
                ...
                currentPattern = getSelfLoopPattern(SymbolicExecutionLab.processedInput, lastNSaves - 1, 0, -1);
